...
def _focus_window(title: str) -> str:
    """Brings a window to focus by title (Windows/Linux/macOS)."""
    ...
    if system == "Windows":
        ...
        try:
            ...
            try:
                ...
                return "Window focus requires wmctrl or xdotool on Linux"
            except Exception as e:
                return f"Could not focus window: {e}"
    
    elif system == "Darwin":  # macOS
        try:
            script = f'tell application "System Events" to activate window (first window whose name contains "{title}")'
            subprocess.run(
                ["osascript", "-e", script],
                capture_output=True, timeout=5
            )
