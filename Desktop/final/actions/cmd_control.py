...
LINUX_COMMAND_MAP = [
    ...
    (["large files", "biggest files", "largest files"],
     "find ~ -type f -exec ls -lh {} \\; 2>/dev/null | sort -k5 -hr | head -10", False),
]

def _find_hardcoded(task: str) -> str | None:
    task_lower = task.lower()
    
    if "edit" in task_lower or "open" in task_lower or any(ext in task_lower for ext in [".txt", ".log", ".md", ".csv"]):
        ...
        if file_match:
            ...
            return f'nano "{filepath}"'
        if any(ed in task_lower for ed in ["edit", "open", "view"]):
            return f'nano "{Path.home() / "Desktop" / "file.txt"}"'
    ...
    for keywords, command, _ in LINUX_COMMAND_MAP:
...
def _ask_gemini(task: str) -> str:
    try:
        ...
        prompt = (
            f"Convert this request to a single Linux/Bash command.\n"
...
def cmd_control(
    ...
    if platform == "linux" and any(x in command.lower() for x in ["nano", "vim", "vi", "gedit", "kate", "mousepad"]):
        subprocess.Popen(command, shell=True)
        return f"Opened: {command}"
    elif platform != "linux" and any(x in command.lower() for x in ["notepad", "explorer", "start "]):
        subprocess.Popen(command, shell=True)
        return f"Opened: {command}"
