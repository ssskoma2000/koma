...
def _ask_gemini_for_desktop_action(task: str) -> str:
    ...
    prompt = f"""You are a desktop automation expert for {platform} systems.
...
- time (sleep only)
- subprocess (ONLY: run with capture_output=True, NO shell=True)
...
- NO file write operations beyond subprocess
- NO exec() or eval()
        ...
        if sys.platform == "win32":
            ...
            try:
                subprocess.run(["gsettings", "set", "org.gnome.desktop.background",
                              "picture-uri-dark", f"file://{path}"], capture_output=True, timeout=5)
                subprocess.run(["gsettings", "set", "org.gnome.desktop.background",
                              "picture-uri", f"file://{path}"], capture_output=True, timeout=5)
                return f"Wallpaper set: {path.name}"
            except FileNotFoundError:
                try:
                    subprocess.run(["dconf", "write", "/org/gnome/desktop/background/picture-uri",
                                  f"'file://{path}'"], capture_output=True, timeout=5)
                    return f"Wallpaper set: {path.name}"
                except FileNotFoundError:
                    return f"Wallpaper setting requires gsettings or dconf on Linux"
    ...
    try:
        if sys.platform == "win32":
            ...
            result = subprocess.run(
                ["osascript", "-e", 'tell application "Finder" to get (get desktop picture as alias)'],
            ...
            try:
                ...
                return "Wallpaper retrieval requires gsettings on Linux"
    except Exception as e:
